...
def sierpinski(degree, p1, p2, p3, color, line_width, screen):
    if degree == -1:
        return
    draw_triangle(p1, p2, p3, color, line_width - 1, screen) # draw the filled triangle
    draw_triangle(p1, p2, p3, (color + 1) % 5, line_width, screen) # draw the triangle border with specific color
    mid12 = find_midpoint(p1, p2)
    mid13 = find_midpoint(p1, p3)
    mid23 = find_midpoint(p2, p3)
    degree -= 1
    sierpinski(degree, p1, mid12, mid13, (color + 2) % 5, line_width, screen)
    sierpinski(degree, mid12, p2, mid23, (color + 4) % 5, line_width, screen)
    sierpinski(degree, mid13, mid23, p3, (color + 6) % 5, line_width, screen)
...
def main():
    ...
    # screen.fill(pygame.Color(25, 25, 112, 255))
    screen.fill(pygame.Color(153, 50, 204, 255))
    pygame.display.flip()
