...
# This function takes a single integer named 'num' as its argument. It
...
def construct_waltz():
    # minuetWaves = [ sa.WaveObject.from_wave_file(minuet_filename(m[roll_dice(2)])) for m in minuet_table ]
    # trioWaves = [ sa.WaveObject.from_wave_file(trio_filename(t[roll_dice(1)])) for t in trio_table ]

    # for mw in minuetWaves:
    #     (mwplay := mw.play()).wait_done()
        
    # for tw in trioWaves:
    #     (twplay := tw.play()).wait_done() 

    minuetFiles = [ minuet_filename(m[roll_dice(2)]) for m in minuet_table ]
    trioFiles = [ trio_filename(t[roll_dice(1)]) for t in trio_table ]

    readMinuetWaves = [ wave.open(mf, 'rb') for mf in minuetFiles ]
    readTrioWaves = [ wave.open(tf, 'rb') for tf in trioFiles ]
    readWaves = readMinuetWaves + readTrioWaves
    audioData = [ readWave.readframes(readWave.getnframes()) for readWave in readWaves ]
    numChannels = [ readWave.getnchannels() for readWave in readWaves ]
    bytesPerSample = [ readWave.getsampwidth() for readWave in readWaves ]
    sampleRates = [ readWave.getframerate() for readWave in readWaves ]

    for i in range(len(readWaves)):
        (playObj := sa.play_buffer(audioData[i], numChannels[i], bytesPerSample[i], sampleRates[i])).wait_done()
